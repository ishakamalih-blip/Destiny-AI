...
def generate_report(data: dict):
    ...
    bp = data.get("behavioral_profile", {})
    thinking = cp.get("thinking_ability") or ("Abstract & Strategic" if cs > 8 else "Logical & Linear" if cs > 6 else "Intuitive")
    iq_type = cp.get("iq_type") or ("Operational reasoning" if cs >= 6 else "Intuitive reasoning")
    career = bp.get("career_guidance") or ("Leadership & Strategy" if cs > 7 and vs > 7 else "Creative Arts & Design" if cs > 7 else "Technical Operations")
    relationship = ep.get("relationship_dynamic") or ("Deeply Empathetic" if es > 7 else "Communicative & Rational" if cs > 7 else "Independent & Strong-willed")
    behavior = bp.get("type") or ("Proactive & Driven" if vs > 7 else "Reflective & Methodical" if cs > 7 else "Adaptable & Fluid")

    c.drawString(140, 575, f"- Thinking Style: {thinking}")
    c.drawString(140, 560, f"- IQ Type: {iq_type}")
